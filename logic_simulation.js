...
const DEFAULT_ITEM_SCORES = {
    groups: { DARK_NEKOME: 1000000000, TREASURE_RADAR: 100000000, VITAN_C: 10000000 },
    items: {
        ...
        "2056": 110, "2055": 90, "0": 10, "1": 10, "2": 10, "3": 10, "4": 10, "5": 10, "6": 10, "7": 10, "8": 10,
...
function getSimAddress(idx) { return ((idx % 2 === 0) ? 'A' : 'B') + ((idx >> 1) + 1); }
...
function runGachaSearch(initialSeed, initialLastId, limits, gachaIds, customWeights = null) {
    ...
    dp[0].set(`0_${startLastId}`, { nodeIdx: 0, lastItemId: startLastId, score: 0, usedN: 0, usedF: 0, usedFG: 0, prev: null, action: null });
    for (let t = 0; t < maxTotal; t++) {
        if (dp[t].size === 0) continue;
        let states = Array.from(dp[t].values());
        if (states.length > BEAM_WIDTH) { states.sort((a, b) => b.score - a.score); states = states.slice(0, BEAM_WIDTH); }
        for (const state of states) {
            for (const gId of gachaIds) {
                ...
                const nextState = {
                    nodeIdx: state.nodeIdx + res.consumed, lastItemId: idStr, score: state.score + point - (weights.costs[type] || 0),
                    usedN: state.usedN + (type === "nyanko" ? 1 : 0), usedF: state.usedF + (type === "fukubiki" ? 1 : 0), usedFG: state.usedFG + (type === "fukubikiG" ? 1 : 0),
                    prev: state, action: { gachaId: gId, gachaName: gachaMaster[gId].name, item: itemMaster[idStr].name, itemId: idStr, isReroll: res.isReroll, addr: res.addr }
                };
    ...
    for (let t = maxTotal; t >= 0; t--) { for (const s of dp[t].values()) { if (s.score > maxScore) { maxScore = s.score; best = s; } } }
    if (best) {
        const path = []; let curr = best;
        ...
        best.path.forEach(p => { const group = ITEM_GROUP_CACHE[p.itemId]; if (best.counts[group] !== undefined) best.counts[group]++; });
    }
    return best;
}
